...
export function InterestsSection({ interests, copy }: InterestsSectionProps) {
  return (
    <section id="interests" aria-labelledby="interests-heading" className="surface-panel rounded-[2.2rem] p-8 transition-colors sm:p-10">
      <p className="text-xs uppercase tracking-[0.3em] text-orange-500 dark:text-orange-300">{copy.eyebrow}</p>
      <h3 id="interests-heading" className="text-heading-theme mt-2 text-3xl font-semibold">
        {copy.title}
      </h3>
      ...
      <div className="mt-6 space-y-4">
        {interests.map((interest) => (
          <article key={interest.title} className="surface-card flex gap-4 rounded-2xl p-4 transition-all duration-300 hover:-translate-y-0.5">
